...
public class homeController {
	...
	/*
	 * @RequestMapping(value = "/signOk", method = RequestMethod.GET) public String getHomeFinal(@RequestParam("em")
	 * String email, @RequestParam("ps") String psd, Model model, HttpSession session) { custCredModel cust =
	 * cdao.getCustomer(email, psd); cdao.updatelastlogin(cust.getCustId()); if (cust != null) {
	 * session.setAttribute("customer", cust); // Store customer object in the session flag = true; } custCredModel
	 * cust1 = (custCredModel) session.getAttribute("customer"); model.addAttribute("cust1", cust1);
	 * model.addAttribute("fl", flag); return "home"; }
	 */
	...
	public String getHomeFinal(@RequestParam("em") String email, @RequestParam("ps") String psd, Model model,
		...
		model.addAttribute("fl", cust != null);
		return "home";
	}
